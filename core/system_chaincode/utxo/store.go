...
import (
	...
	"github.com/hyperledger/fabric/core/system_chaincode/utxo/util"
)

// Store is used to abstract the ChaincodeStub so we can store either in the
// DB or memory for testing
type Store struct {
	stub *shim.ChaincodeStub
}

// MakeChaincodeStore creates a new chaincode store
...
// GetState retirns value for a given key
...
// DelState delets the value for a given key
func (s *Store) DelState(key util.Key) error {
	return s.stub.DelState(keyToString(&key))
}

// PutState sets the value for a given key
...
// GetTran returns the transa tion for a given key
...
// PutTran sets a transaction for a given key
